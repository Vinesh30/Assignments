...
public class EmployeeController {
	...
	public ModelAndView updateEmployee(@ModelAttribute("employee") Employee employee) throws Exception {
		...
		/*
		 * ResultSet set=employeeDao.getDetails(); List<Employee> list = new
		 * ArrayList<>(); while(set.next()) { Employee e = new Employee();
		 * e.setEmployeeId(set.getInt(1)); e.setEmployeeName(set.getString(2));
		 * e.setEmployeeDepartment(set.getString(3));
		 * e.setEmployeeDesignation(set.getString(4));
		 * e.setEmployeeSalary(set.getInt(5)); list.add(e); }
		 */
